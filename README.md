...
レベルに合った短い英語を、スクリプトを隠したまま通しで聴くリスニング練習。3つのレベル帯（入門/中級/上級）× 3ドメイン（日常/ビジネス/IT）で均等に用意した42本（各0.9〜1.5分）を収録し、音声はすべてリポジトリに同梱済み（OpenAI TTS で事前生成）。音声の準備中でも停止や画面移動が確実に反映され、あとから古い音声が鳴り始めることはありません。聴き終えたらボタンでスクリプト表示・日本語訳と表現解説も確認できます。聴いた本数は「今週◯本」の情報表示のみでノルマはありません。記録の保存に失敗しても聴取完了や感想入力には影響せず、画面の案内から同じ聴取を二重計上せずに再試行できます。素材は生成CLIで自分のレベルに合わせてさらに増やせます。
