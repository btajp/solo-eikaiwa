...
GitHub Releases から dmg をダウンロードして起動するだけで動く、ネイティブウィンドウ版のアプリです（Apple Silicon Mac 専用）。サーバ本体・教材コンテンツ・音声認識エンジンをアプリに同梱しているため、Bun や Homebrew のセットアップは不要です（初回のみ音声認識モデルのダウンロードが必要）。機能・データはブラウザ版と共通です。入手方法・初回セットアップの詳しい説明は後述「起動: デスクトップアプリ（Tauri）」を参照してください。
...
5. **初回のマイク許可は必ず Finder から `.app` をダブルクリックして起動した状態で行ってください。** 録音ボタンを押すと表示されるダイアログの請求元が正しく「solo-eikaiwa」になっていることを確認してから許可してください（ターミナル経由の起動だと請求元表示がずれる現象を実機で確認しているため）
...
**マイク許可の再表示について**: アプリを新しいバージョンに置き換えると、マイクの許可を再度求められることがあります。ad-hoc 署名は更新のたびに署名情報が変わる仕様のためで、想定内の挙動です。
