...
レベルに合った短い英語を、スクリプトを隠したまま通しで聴くリスニング練習。3つのレベル帯（入門/中級/上級）× 3ドメイン（日常/ビジネス/IT）で均等に用意した42本（各0.9〜1.5分）を収録し、音声はすべてリポジトリに同梱済み（OpenAI TTS で事前生成）。再生速度は **0.7x / 0.85x / 1x / 1.25x** から選べ、再生中の切り替えも即反映されます（声の高さは変わりません）。同梱音声の話速は実測で平均約190WPMのため、入門帯では 0.7x 前後に落として「理解できる速さ」で聴くのがおすすめです。音声の準備中でも停止や画面移動が確実に反映され、あとから古い音声が鳴り始めることはありません。聴き終えたらボタンでスクリプト表示・日本語訳と表現解説も確認できます。聴いた本数は「今週◯本」の情報表示のみでノルマはありません。記録の保存に失敗しても聴取完了や感想入力には影響せず、画面の案内から同じ聴取を二重計上せずに再試行できます。素材は生成CLIで自分のレベルに合わせてさらに増やせます。
...
| 多聴（1本・0.9〜1.5分） | 同梱音声のため課金対象なし | 0円 |
